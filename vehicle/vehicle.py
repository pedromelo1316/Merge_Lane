...
def main():
    """Entry point do veículo: lê variáveis de ambiente, liga ao Vanetza e ao coordinator,
    ...
    ts = time.strftime("%H:%M:%S")
    print(f"[{ts}] [{vehicle_id}] VEHICLE_ID={vehicle_id} STATION_ID={station_id}")
    print(f"[{ts}] [{vehicle_id}] VANETZA_ZENOH_URL={vanetza_url}")
    print(f"[{ts}] [{vehicle_id}] COORDINATOR_ZENOH_URL={coord_url}")
    ...
    def on_scenario(sample):
        ...
        try:
            ...
            stop_event.set()         # aborta cenário actual (se houver)
            scenario_queue.put(data) # enfileira o novo
    ...
    print(f"[{time.strftime('%H:%M:%S')}] [{vehicle_id}] Pronto. A aguardar cenário...")
    ...
    while True:
        ...
        ts = time.strftime("%H:%M:%S")
        print(f"[{ts}] [{vehicle_id}] Cenário recebido: '{name}'")
